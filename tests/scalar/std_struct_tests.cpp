...
TEST(scalar_struct, add_array) {
	const scalar_struct test { glsl_value<int, 3>("test", { 2, 3, 4 }) };
	EXPECT_EQ(test.get<int>("test[1]"), 3);
	EXPECT_EQ(test.get_offset("test"), 0);
}

TEST(scalar_struct, set_get_test) {
	// clang-format off
	scalar_struct subTest {
	...
	scalar_struct test {
		...
		glsl_value<scalar_struct>("struct", subTest),
		glsl_value<scalar_struct, 2>("struct_array", subTest.get_layout())
	};
	// clang-format on
...
#pragma region SET_STRUCT
	EXPECT_EQ(test.get<scalar_struct>("struct", subTest.get_layout()), subTest);
...
#pragma region SET_STRUCT_ARRAY
	std::vector<scalar_struct> struct_array { scalar_struct(subTest.get_layout()), scalar_struct(subTest.get_layout()) };
	EXPECT_EQ(test.get<std::vector<scalar_struct>>("struct_array", subTest.get_layout()), struct_array);
...
TEST(scalar_struct, writer) {
	// clang-format off
	scalar_struct subSubTest {
		glsl_value<int>("b")
	};

	scalar_struct subTest {
		glsl_value<bool>("a", true),
		glsl_value<scalar_struct>("c", subSubTest)
	};
...
TEST(scalar_struct, parser) {
	...
	glsl_vulkan_writer vkWriter;
	vkWriter.append_shader_storage_buffer(0, 0, "SSBO", "ssbo", test, "readonly");

	parser.add_structs_definitions(vkWriter.to_string());
	...
	// std::cout << vkWriter.to_string() << std::endl;
#endif
}

TEST(scalar_struct, copy_test) {
	scalar_struct test { glsl_value<int, 3>("test", { 2, 3, 4 }) };
	int value = test.get<int>("test[1]");
	EXPECT_EQ(value, 3);
	size_t ret = test.get_offset("test");
	EXPECT_EQ(ret, 0);

	const scalar_struct test1 = test;
...
TEST(scalar_struct, constructor_and_get) {
	size_t ret;
...
#pragma region UIElement
	// clang-format off
	scalar_struct uiElement {
		glsl_value<scalar_struct>("rect", rect),
		glsl_value<scalar_struct>("sprite", sprite),
		glsl_value<scalar_struct>("fill", fill),
