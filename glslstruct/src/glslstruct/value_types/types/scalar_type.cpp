...
scalar_type::scalar_type(ValueType type) noexcept 
	: value_type(get_value_type_size(type)), _type(type) {}
...
ValueType scalar_type::get_type() const noexcept {
	return _type;
}

std::string scalar_type::to_string() const noexcept {
	return glslstruct::to_string(_type);
}

bool glslstruct::operator==(const scalar_type& lhs, const scalar_type& rhs) noexcept {
...
size_t std::hash<scalar_type>::operator()(const scalar_type& value) const noexcept {
