...
size_t std430_layout::_get_array_elem_base_alignment(size_t baseAlignment) const noexcept { return baseAlignment; }
...
std::vector<size_t> std430_layout::add(const std::string_view& name, const std430_layout& structLayout, size_t count) {
	return _add_struct_array(name, structLayout.base_alignment(), structLayout._currentOffset, structLayout._values, count);
}

size_t std430_layout::base_alignment() const noexcept { return _maxAlignment; }

	#if _GLSL_STRUCT_HAS_CXX20
bool std430_layout::operator==(const std430_layout& other) const noexcept = default;
bool std430_layout::operator!=(const std430_layout& other) const noexcept = default;
	#else
bool std430_layout::operator==(const std430_layout& other) const noexcept { return base_layout::operator==(other); }
