...
void base_layout::_set_variable(const std::string_view& name, size_t alignmentOffset, const base_type_handle& type, size_t size,
  size_t padding) {
...
void base_layout::_update_max_alignment(size_t baseAlignment) noexcept { _maxAlignment = std::max(baseAlignment, _maxAlignment); }
...
std::vector<size_t> base_layout::_add_array(const std::string_view& name, size_t arraySize, size_t baseAlignment,
  size_t baseOffset, const base_type_handle& type) {
		// CHECK SIZE
		if (arraySize == 0) { return std::vector<size_t>(); }
	...
	size_t alignmentOffset = _ceil_to_nearest_multiple(_currentOffset, baseAlignment);
		...
		// CHECK OVERFLOW
		if (alignmentOffset < _currentOffset) { return std::vector<size_t>(); }
	...
	_set_variable(name, alignmentOffset, std::make_shared<array_type>(type, arraySize), arrayTotalAlignment, 0);
...
size_t base_layout::_add_vec(const std::string_view& name, size_t length, size_t baseAlignment, size_t baseOffset,
  ValueType type) {
		if (length < 2 || length > 4) { return bad_offset(); }

	const size_t vecSize = length * baseOffset;

		if (length != 3) { return _add(name, length * baseAlignment, vecSize, std::make_shared<vec_type>(type, length)); }
	return _add(name, (length + 1) * baseAlignment, vecSize, std::make_shared<vec_type>(type, length));
}
...
std::vector<size_t> base_layout::_add_vec_array(const std::string_view& name, size_t length, size_t baseAlignment,
  size_t baseOffset, ValueType type, size_t arraySize) {
		if (length < 2 || length > 4) { return std::vector<size_t>(); }

	const size_t vecSize = length * baseOffset;

		if (length != 3) {
			return _add_array(name, arraySize, length * baseAlignment, vecSize, std::make_shared<vec_type>(type, length));
		}
	return _add_array(name, arraySize, (length + 1) * baseAlignment, vecSize, std::make_shared<vec_type>(type, length));
}

size_t base_layout::_add_mat(const std::string_view& name, size_t columns, size_t rows, MajorType major, size_t baseAlignment,
  size_t baseOffset, ValueType type) {
	...
	size_t alignment =
	  _get_array_elem_base_alignment((vecSize == 3) ? ((vecSize + 1) * baseAlignment) : (vecSize * baseAlignment));
	...
	// ADD VEC ARRAY
	std::vector<size_t> vecsOffsets = _add_vec_array(name, vecSize, baseAlignment, baseOffset, type, arraySize);

	// CHANGE ARRAY TYPE TO MAT TYPE
	value_data& varData				= _values.at(name.data());

	varData.change_type(std::make_shared<mat_type>(type, columns, rows, major));
...
std::vector<size_t> base_layout::_add_mat_array(const std::string_view& name, size_t columns, size_t rows, MajorType major,
  size_t baseAlignment, size_t baseOffset, ValueType type, size_t arraySize) {
	...
	std::vector<size_t> values(arraySize);
		for (size_t i = 0; i != arraySize; ++i) {
			values[i] = _add_mat(_get_array_elem_name(name, i), columns, rows, major, baseAlignment, baseOffset, type);
		}

	// SET ARRAY BEGIN POINTER
	_set_variable(name, values[0], std::make_shared<array_type>(type, columns, rows, major, arraySize), alignment, 0);
...
size_t base_layout::_add_struct(const std::string_view& name, size_t baseAlignment, size_t baseOffset,
  const std::unordered_map<std::string, value_data>& values) {
	size_t alignmentOffset = _add(name, baseAlignment, baseOffset, std::make_shared<struct_type>(values, baseOffset));

		// CHECK RESULT
		if (alignmentOffset == bad_offset()) { return alignmentOffset; }

		for (const auto& [value_name, data] : values) {
			_set_variable(_get_struct_elem_name(name, value_name), alignmentOffset + data.get_offset(), data.get_type(),
...
std::vector<size_t> base_layout::_add_struct_array(const std::string_view& name, size_t baseAlignment, size_t baseOffset,
  const std::unordered_map<std::string, value_data>& values, size_t arraySize) {
	...
	// SET ARRAY BEGIN POINTER
	_set_variable(name, valuesOffsets[0], std::make_shared<array_type>(values, baseOffset, arraySize), arrayTotalSize, 0);
...
base_layout::base_layout(const base_layout& other) noexcept
	: _values(other._values), _currentOffset(other._currentOffset), _maxAlignment(other._maxAlignment) {}

base_layout::base_layout(base_layout&& other) noexcept
	: _values(std::exchange(other._values, {})), _currentOffset(other._currentOffset), _maxAlignment(other._maxAlignment) {}
...
size_t base_layout::bad_offset() noexcept { return ~static_cast<size_t>(0); }
...
value_data base_layout::get(const std::string_view& name) const noexcept {
		if (!contains(name)) { return value_data(bad_offset(), nullptr, 0, 0); }

	return _values.at(name.data());
}

size_t base_layout::get_offset(const std::string_view& name) const noexcept { return get(name).get_offset(); }
...
const base_type_handle& base_layout::get_type(const std::string_view& name) const noexcept { return get(name).get_type(); }

size_t base_layout::get_total_size(const std::string_view& name) const noexcept { return get(name).get_total_size(); }

size_t base_layout::get_size(const std::string_view& name) const noexcept { return get(name).get_size(); }

size_t base_layout::get_padding(const std::string_view& name) const noexcept { return get(name).get_padding(); }
...
size_t base_layout::padding() const noexcept { return size() - _currentOffset; }
...
bool base_layout::operator!=(const base_layout& other) const noexcept = default;
	#else
bool base_layout::operator!=(const base_layout& other) const noexcept { return !(*this == other); }
	#endif
