...
size_t std140_layout_traits::get_scalar_alignment(const ValueType valueType, context_type&) noexcept {
	return get_value_type_size(valueType);
}

size_t std140_layout_traits::get_vec_alignment(const ValueType valueType, const size_t length, context_type&) noexcept {
...
void std140_layout_traits::after_add(size_t&, const size_t, const size_t alignment, context_type& ctx) noexcept {
	ctx.update_max_alignment(alignment);
}

void std140_layout_traits::after_add_struct(size_t& currentOffset, const size_t size, const size_t alignment,
  context_type& ctx) noexcept {
	ctx.structAlignment = alignment;
	after_add(currentOffset, size, alignment, ctx);
}

size_t std::hash<std140_layout_context>::operator()(const std140_layout_context& ctx) const noexcept {
