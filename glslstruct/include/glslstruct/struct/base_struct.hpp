...
namespace glslstruct {
	...
	private:
		...
		_GLSL_STRUCT_CONSTEXPR20 std::vector<size_t> _convert_array(const std::string_view& name, const _Start* values,
		  size_t size, const mstd::func_t<std::vector<size_t>(const std::string_view&, const _Conv*, size_t)>& arrayFunc) {
				if _GLSL_STRUCT_CONSTEXPR17 (std::is_same_v<_Start, _Conv>) { return arrayFunc(name, values, size); }
				else {
					std::vector<_Conv> convertedValues;
					convertedValues.reserve(size);
						for (size_t i = 0; i != size; ++i) { convertedValues.push_back(static_cast<_Conv>(values[i])); }
		...
		_GLSL_STRUCT_CONSTEXPR17 void _add_multiple(const glsl_value<T, num>& value, const glsl_value<Ts, nums>&... values) {
				...
				if _GLSL_STRUCT_CONSTEXPR17 (sizeof...(Ts) > 0 && sizeof...(nums) > 0) { _add_multiple(values...); }
		}
		...
		_GLSL_STRUCT_CONSTEXPR17 void _add_value_data(const std::vector<std::byte>& valueData, size_t valueOffset) {
			const size_t dataAvailableSpace = _data.size() - valueOffset;
				...
				if (valueOffset < _data.size()) {
					std::copy_n(valueData.begin(), std::min(dataAvailableSpace, valueData.size()), _data.begin() + valueOffset);
				}
		...
		_GLSL_STRUCT_CONSTEXPR17 size_t _add(const std::string_view& name, const T& value) {
			...
			_data.reserve(_layout.size());
				...
				// CHECK VALUE PADDING
				if (_data.size() < valueOffset) { _data.resize(valueOffset); }

			// SET VALUE DATA
			_add_value_data(_get_value_data(value), valueOffset);

				// UPDATE SIZE
				if (_data.size() < _data.capacity()) { _data.resize(_data.capacity()); }
		...
		_GLSL_STRUCT_CONSTEXPR20 std::vector<size_t> _add_array(const std::string_view& name, const T* values, size_t size) {
			...
			std::vector<size_t> valuesOffsets = _layout.template add<T>(name, size);
				...
				// CHECK ERROR
				if (valuesOffsets.size() == 0) { return valuesOffsets; }
			...
			_data.reserve(_layout.size());
				...
				for (size_t i = 0; i < valuesOffsets.size() && i < size; ++i) {
						// CHECK VALUE PADDING
						if (_data.size() < valuesOffsets[i]) { _data.resize(valuesOffsets[i]); }
		...
		_GLSL_STRUCT_CONSTEXPR17 void _set_value_data(const std::vector<std::byte>& valueData, size_t valueOffset) noexcept {
			std::copy_n(valueData.begin(), std::min(valueData.size(), _data.size() - valueOffset), _data.begin() + valueOffset);
		}

		_GLSL_STRUCT_CONSTEXPR17 void _set_array_value_data(const std::vector<std::byte>& valueData, size_t valueOffset,
		  size_t arrayElemSize) noexcept {
			std::copy_n(valueData.begin(), std::min(arrayElemSize, _data.size() - valueOffset), _data.begin());
		}
		...
		_GLSL_STRUCT_CONSTEXPR20 bool _set_array(const std::string_view& name, const T* values, size_t size) {
			...
			std::vector<size_t> valuesOffsets = _layout.get_array_offsets(name);
				...
				if (valuesOffsets.size() == 0) { return false; }
		...
		_GLSL_STRUCT_CONSTEXPR20 bool _set_struct_array(const std::string_view& name, const std::vector<std::byte>* values,
		  size_t size) {
				// CHECK SIZE
				if (size == 0) { return false; }
		...
		_GLSL_STRUCT_CONSTEXPR20 std::vector<std::byte> _get_value_data(size_t valueOffset, size_t valueSize) const {
			...
			std::copy_n(_data.begin() + valueOffset, std::min(valueSize, _data.size() - valueOffset), valueData.begin());
			return valueData;
		}
		...
		_GLSL_STRUCT_CONSTEXPR20 std::vector<T> _get_array(const std::string_view& name) const {
				// CHECK VARIABLE
				if (!contains(name)) { return std::vector<T>(); }

			// GET VALUES OFFSETS
			std::vector<size_t> valuesOffsets = _layout.get_array_offsets(name);

				// CHECK ARRAY ELEMENTS OFFSETS
				if (valuesOffsets.size() == 0) { return std::vector<T>(); }
			...
			size_t maxSize = 0;
				for (size_t i = 0; i < valuesOffsets.size(); ++i) {
					...
					std::copy_n(_data.begin() + valuesOffsets[i], maxSize, valueData.begin());
						...
						if (maxSize < sizeof(T)) { std::fill_n(valueData.begin() + maxSize, sizeof(T), T()); }
		...
		_GLSL_STRUCT_CONSTEXPR17 base_struct _get_struct(const std::string_view& name, const Layout& layout) const {
				// CHECK VARIABLE
				if (!contains(name)) { return base_struct(layout); }
			...
			// SET VALUE DATA
			std::copy_n(_data.begin() + valueOffset, std::min(layout.size(), _data.size() - valueOffset), value._data.begin());

			// RETURN VALUE
			return value;
		}

		_GLSL_STRUCT_CONSTEXPR20 std::vector<base_struct> _get_struct_array(const std::string_view& name,
		  const Layout& layout) const {
				// CHECK VARIABLE
				if (!contains(name)) { return std::vector<base_struct>(); }

			// GET VALUES OFFSETS
			std::vector<size_t> valuesOffsets = _layout.get_array_offsets(name);

				// CHECK ARRAY ELEMENTS OFFSETS
				if (valuesOffsets.size() == 0) { return std::vector<base_struct>(); }
		...
		_GLSL_STRUCT_CONSTEXPR17 std::vector<_Conv> _get_array(const std::string_view& name,
		  const mstd::func_t<std::vector<_Start>(const std::string&)>& getArrayFunc) {
				if _GLSL_STRUCT_CONSTEXPR17 (std::is_same_v<_Start, _Conv>) { return getArrayFunc(name); }
				else {
					std::vector<_Start> values = getArrayFunc(name);
					std::vector<_Conv> convertedValues;
					convertedValues.reserve(values.size());
						for (auto& val : values) { convertedValues.push_back(static_cast<_Conv>(val)); }
	...
	public:
		...
		_GLSL_STRUCT_CONSTEXPR17 size_t add(const std::string_view name, const T& value) {
				if _GLSL_STRUCT_CONSTEXPR17 (std::is_same_v<T, bool>) { return _add(name, static_cast<unsigned int>(value)); }
				else { return _add(name, value); }
		}
		...
		_GLSL_STRUCT_CONSTEXPR20 std::vector<size_t> add(const std::string_view name, const T* values, size_t size) {
			...
			return _convert_array<T, type>(name, values, size,
			  [&](const std::string_view& name, const type* values, size_t size) -> std::vector<size_t> {
				  return _add_array(name, values, size);
			  });
		}
		...
		_GLSL_STRUCT_CONSTEXPR17 size_t add(const std::string_view name, const M& value) {
			...
			_GLSL_STRUCT_CONSTEXPR17 const size_t R = glsl_mat_rows<M>;
				...
				if _GLSL_STRUCT_CONSTEXPR17 (major == MajorType::Column) {
						...
						else { return _add(name, glm::transpose(value)); }
				}
		}
			...
			_GLSL_STRUCT_CONSTEXPR17 const size_t R = glsl_mat_rows<M>;
				...
				if _GLSL_STRUCT_CONSTEXPR17 (major == MajorType::Column) {
					...
					std::vector<glm::mat<R, C, T> > transposedValues(size);
						for (size_t i = 0; i < size; ++i) { transposedValues.push_back(glm::transpose(values[i])); }
		...
		#if _GLSL_STRUCT_HAS_CXX20
		template<utils::glsl_mat M, MajorType major = MajorType::Column, size_t N>
		#else
		template<class M, MajorType major = MajorType::Column, size_t N, std::enable_if_t<utils::is_glsl_mat_v<M>, bool> = true>
		#endif
			...
			_GLSL_STRUCT_CONSTEXPR17 const size_t R = glsl_mat_rows<M>;
				...
				if _GLSL_STRUCT_CONSTEXPR17 (major == MajorType::Column) {
					...
					std::vector<glm::mat<R, C, T> > transposedValues(N);
						for (size_t i = 0; i < N; ++i) { transposedValues.push_back(glm::transpose(values[i])); }
			...
			_GLSL_STRUCT_CONSTEXPR17 const size_t R = glsl_mat_rows<M>;
				...
				if _GLSL_STRUCT_CONSTEXPR17 (major == MajorType::Column) {
					...
					std::vector<glm::mat<R, C, T> > transposedValues(values.size());
						for (size_t i = 0; i < values.size(); ++i) { transposedValues.push_back(glm::transpose(values[i])); }

					using type = glm::mat<R, C, std::conditional_t<std::is_same_v<T, bool>, unsigned int, T> >;

					return _convert_array<M, type>(name, transposedValues, values.size(),
					  [&](const std::string_view name, const type* values, size_t size) -> std::vector<size_t> {
						  return _addArray(name, values, size);
					  });
				}
		}
		...
		_GLSL_STRUCT_CONSTEXPR17 bool set(const std::string_view name, const T& value) {
				if _GLSL_STRUCT_CONSTEXPR17 (std::is_same_v<T, bool>) { return _set(name, static_cast<unsigned int>(value)); }
				else { return _set(name, value); }
		}
		...
		_GLSL_STRUCT_CONSTEXPR17 bool set(const std::string_view name, const T* values, size_t size) {
			...
			return _convert_array<T, type>(name, values, size,
			  [&](const std::string_view name, const type* values, size_t size) -> bool {
				  return _set_array(name, values, size);
			  });
		}
		#if _GLSL_STRUCT_HAS_CXX20
		template<utils::glsl_scalar T, size_t N>
		...
		_GLSL_STRUCT_CONSTEXPR17 bool set(const std::string_view name, const M& value) {
			...
			_GLSL_STRUCT_CONSTEXPR17 const size_t R = glsl_mat_rows<M>;
				...
				if _GLSL_STRUCT_CONSTEXPR17 (major == MajorType::Column) {
						...
						else { return _set(name, glm::transpose(value)); }
				}
		}
		...
		[[nodiscard]] _GLSL_STRUCT_CONSTEXPR17 T get(const std::string_view name) const {
				if _GLSL_STRUCT_CONSTEXPR17 (std::is_same_v<T, bool>) { return static_cast<T>(_get<unsigned int>(name)); }
				else { return _get<T>(name); }
		}
		...
		_GLSL_STRUCT_CONSTEXPR17 void get(const std::string_view name, T*& valuesDest, size_t size) const {
			std::vector<T> values;
				...
				else { values = _get_array<T>(name); }
			std::copy_n(values.begin(), std::min(values.size(), size), valuesDest);
				if (size > values.size()) { std::fill_n(valuesDest + values.size(), size - values.size(), T()); }
			values.clear();
		}
		...
		_GLSL_STRUCT_CONSTEXPR17 void get(const std::string_view name, V*& valuesDest, size_t size) const {
			...
			std::vector<V> values;
				...
				else { values = _get_array<V>(name); }

			std::copy_n(values.begin(), std::min(values.size(), size), valuesDest);
				if (size > values.size()) { std::fill_n(valuesDest + values.size(), size - values.size(), V()); }
			values.clear();
		}
		...
		[[nodiscard]] _GLSL_STRUCT_CONSTEXPR17 M get(const std::string_view name) const {
			...
			_GLSL_STRUCT_CONSTEXPR17 const size_t R = glsl_mat_rows<M>;
				...
				if _GLSL_STRUCT_CONSTEXPR17 (major == MajorType::Column) {
						...
						else { return _get<M>(name); }
				}
				else {
						if _GLSL_STRUCT_CONSTEXPR17 (std::is_same_v<T, bool>) {
							return static_cast<M>(glm::transpose(_get<glm::mat<R, C, unsigned int> >(name)));
						}
						else { return glm::transpose(_get<glm::mat<R, C, T> >(name)); }
				}
		}
			...
			std::vector<M> values;
				if _GLSL_STRUCT_CONSTEXPR17 (major == MajorType::Column) {
					...
					std::vector<transposedType> transposedValues;
						...
						else { transposedValues = _get_array<transposedType>(name); }

					values.reserve(transposedValues.size());
						for (auto& value : transposedValues) { values.push_back(glm::transpose(value)); }
					transposedValues.clear();
				}

			std::copy_n(values.begin(), std::min(values.size(), size), valuesDest);
				if (size > values.size()) { std::fill_n(valuesDest + values.size(), size - values.size(), M()); }
			values.clear();
		}

		#if _GLSL_STRUCT_HAS_CXX20
		template<utils::glsl_mats_vector MV, MajorType major = MajorType::Column>
		#else
		template<class MV, MajorType major = MajorType::Column, std::enable_if_t<utils::is_glsl_mats_vector_v<MV>, bool> = true>
		#endif
		...
		_GLSL_STRUCT_CONSTEXPR17 void get(const std::string_view name, const Layout& layout, S*& valuesDest, size_t size) const {
			...
			std::copy_n(values.begin(), std::min(values.size(), size).valuesDest);
				if (size > values.size()) { std::fill_n(valuesDest + values.size(), size - values.size(), base_struct(layout)); }
			values.clear();
		}
