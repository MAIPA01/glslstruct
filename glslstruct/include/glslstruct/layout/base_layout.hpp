...
namespace glslstruct {
	...
	private:
		...
		void _update_max_alignment(size_t baseAlignment) noexcept { _maxAlignment = std::max(baseAlignment, _maxAlignment); }

		#if _GLSL_STRUCT_HAS_TYPES
		[[nodiscard]] size_t _add(std::string_view name, size_t baseAlignment, size_t baseOffset, const base_type_handle& type) {
		#else
		...
		[[nodiscard]] std::vector<size_t> _add_scalar_array(std::string_view name, ValueType valueType, size_t count) {
			...
			return _add_array(name, count, traits_type::get_scalar_alignment(valueType), scalarSize,
			  std::make_shared<scalar_type>(valueType, scalarSize));
		#else
			return _add_array(name, count, traits_type::get_scalar_alignment(valueType), get_value_type_size(valueType));
		#endif
		}
		...
		[[nodiscard]] std::vector<size_t> _add_vec_array(std::string_view name, size_t length, ValueType valueType,
		  size_t count) {
			...
			return _add_array(name, count, traits_type::get_vec_alignment(valueType, length), vecSize,
			  std::make_shared<vec_type>(valueType, length, vecSize));
		#else
			return _add_array(name, count, traits_type::get_vec_alignment(valueType, length), get_value_type_size(valueType)
		#endif
		}
		...
		[[nodiscard]] size_t _add_mat(std::string_view name, size_t columns, size_t rows, ValueType valueType) {
			...
			size_t matSize			= _ceil_to_nearest_multiple(_currentOffset, alignment);
				...
				if (mstd::mul_overflow(alignment, arrayCount, alignment) || mstd::add_overflow(matSize, alignment, matSize)) {
					glsl_struct_assert(false, "data overflow would happen!");
					return bad_offset();
				}
		...
		[[nodiscard]] std::vector<size_t> _add_mat_array(std::string_view name, size_t columns, size_t rows, ValueType valueType,
		  size_t count) {
				// CHECK ARRAY SIZE
				if (count == 0) {
			...
			size_t matArrayAlignment = traits_type::get_array_alignment(traits_type::get_mat_alignment(valueType, columns, rows));
		...
		// SET ARRAY BEGIN POINTER
		#if _GLSL_STRUCT_HAS_TYPES
			_set_variable(name, values[0], std::make_shared<array_type>(valueType, columns, rows, count, matArraySize), alignment,
			  0);
		#else
		...
		[[nodiscard]] virtual std::vector<size_t> _add_struct_array(std::string_view name, size_t baseAlignment,
		  size_t baseOffset, const std::unordered_map<std::string, struct_elem_data>& values, size_t arraySize) {
				// CHECK ARRAY SIZE
		...
		_GLSL_STRUCT_CONSTEXPR17 void _add_variables(const glsl_variable<T, num>& var,
		  const glsl_variable<Ts, nums>&... vars) noexcept {
				...
				if _GLSL_STRUCT_CONSTEXPR17 (sizeof...(Ts) > 0 && sizeof...(nums) > 0) { _add_multiple(vars...); }
		}
	...
	public:
		...
		[[nodiscard]] value_data get(std::string_view name) const noexcept;

		[[nodiscard]] size_t get_offset(std::string_view name) const noexcept;
		[[nodiscard]] std::vector<size_t> get_array_offsets(std::string_view name) const noexcept;

		[[nodiscard]] const base_type_handle& get_type(std::string_view name) const noexcept;
		#if _GLSL_STRUCT_HAS_CXX20
		...
		[[nodiscard]] size_t get_total_size(std::string_view name) const noexcept;
		[[nodiscard]] size_t get_size(std::string_view name) const noexcept;
		[[nodiscard]] size_t get_padding(std::string_view name) const noexcept;
