...
namespace glslstruct {
	...
	template<class T>
	concept type_visitor = requires {
		{ std::declval<T&>().visit(std::declval<const scalar_type&>()) } -> std::same_as<void>;
		{ std::declval<T&>().visit(std::declval<const vec_type&>()) } -> std::same_as<void>;
		{ std::declval<T&>().visit(std::declval<const mat_type&>()) } -> std::same_as<void>;
		{ std::declval<T&>().visit(std::declval<const struct_type&>()) } -> std::same_as<void>;
		{ std::declval<T&>().visit(std::declval<const array_type&>()) } -> std::same_as<void>;
	};
	...
	template<class T>
	struct is_type_visitor<T, std::void_t<
		std::enable_if_t<std::is_same_v<void, decltype(std::declval<T&>().visit(std::declval<const scalar_type&>()))>>,
